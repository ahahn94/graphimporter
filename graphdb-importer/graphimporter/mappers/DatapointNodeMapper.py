...
class DatapointNodeMapper(DatapointDtoMapper):

    def entity_to_dto_string(self, entity: Datapoint):
    ...
    def entity_to_dto_identifiers_string(self, entity: Datapoint):
        template = "Datapoint {{{coordinates}}}"
        return template.format(coordinates=self.__get_prepared_coordinates_string(entity))

    def dto_to_entity(self, node):
    ...
    def dtos_to_entities(self, nodes_list):
        entities = []
        for node in nodes_list:
            entities.append(self.dto_to_entity(node))
        return entities
